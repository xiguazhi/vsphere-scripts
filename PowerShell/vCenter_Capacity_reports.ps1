...
#-------------------------CHANGE THESE VALUES--------------------------------
$SMTPServer = "smtp.extendhealth.com" #"webmail.interpublic.com"
$vCenterServerName = "192.168.2.58"
...
Function Create-Chart() {
	...
	$FullPath = ("$env:temp\" + $FileName + ".png")
...
function Out-PDFFile
{
    ...
    begin
    {
        #Check to see whether PDF printer was setup correctly
        $printerName = 'PrintPDFUnattended'
        #File the print driver always prints to.
        ...
        # Check if printer exists & add printer if it doesn't
        $printer = Get-Printer -Name $printerName -ErrorAction SilentlyContinue
        if (!$?)
        {
            $tempPDF = "$env:temp\PDFResult.pdf"
        ...
        else
        {
            ...
            if ($tempPDF -notlike '?:\*')
            {
                throw "Printer $printerName is not setup correctly. Remove the printer and try again..."
            }
        }

        # specify the path to the file you want to create
        # (adjust if you want)
        $outPath = "$home\Desktop\results.pdf"
...
#Show New PDF file - This is where a slack Message would most likely go, Or a jira ticket web request. / email request.
    if ($Open)
    {
...
$HTMLPage | Out-File ( "$env:temp\report.html") # Export locally to html file
#Save path of html file
$html_file = "$env:temp\report.html"
Write-Host "Report has exported to HTML file " ("$env:temp\report.html")
#Export html file to PDF and ship PDF to owners of vCenter management.
wkhtmltopdf $html_file "$env:temp\report.pdf"
...
Write-Host "One moment while report is being printed to PDF"
